...
const PRESETS: { id: string; tr: string; en: string; w: AdvisorWeights }[] = [
  {
    ...
    tr: "Dengeli",
    en: "Balanced",
    w: { intelligence: 50, speed: 50, price: 50, context: 40 },
  },
  {
    id: "smartest",
    tr: "En akıllı",
    en: "Smartest",
    w: { intelligence: 100, speed: 20, price: 15, context: 40 },
  },
  {
    id: "cheapest",
    tr: "En ekonomik",
    en: "Most economical",
    w: { intelligence: 30, speed: 40, price: 100, context: 25 },
  },
  {
    id: "fastest",
    tr: "En hızlı",
    en: "Fastest",
    w: { intelligence: 35, speed: 100, price: 45, context: 20 },
  },
  {
    id: "longctx",
    tr: "Uzun bağlam",
    en: "Long context",
    w: { intelligence: 55, speed: 25, price: 30, context: 100 },
  },
];
...
export default function Advisor() {
  ...
  const [req, setReq] = useState<AdvisorReq>({
    vision: false,
    audio: false,
    tools: false,
    reasoning: false,
  ...
  const setW = (k: keyof AdvisorWeights, v: number) =>
    setWeights(p => ({ ...p, [k]: v }));
  const setR = (k: keyof AdvisorReq, v: boolean) =>
    setReq(p => ({ ...p, [k]: v }));

  const results = useMemo(() => {
    if (!data || !bounds) return [];
    return scoreModels(data.models, weights, req, usecase, bounds, lang).slice(
      0,
      12
    );
        ...
        <div className="grid lg:grid-cols-[360px_1fr] gap-6">
          ...
          <aside className="lg:sticky lg:top-20 lg:self-start">
            <div className="rounded-xl border border-border/60 bg-card/40 p-5 space-y-6">
              ...
              <div>
                <h3 className="font-mono text-[11px] uppercase tracking-[0.16em] text-muted-foreground mb-4">
                  {t("adv_priority")}
                </h3>
                <div className="space-y-4">
                  <WeightRow
                    ...
                    onChange={v => setW("intelligence", v)}
                    color="var(--primary)"
                  />
                  <WeightRow
                    icon={Zap}
                  ...
                  <WeightRow
                    ...
                    label={t("adv_w_price")}
                    value={weights.price}
                    onChange={v => setW("price", v)}
                    color="oklch(0.7 0.16 145)"
                  />
                  <WeightRow
                    icon={ScrollText}
                    label={t("adv_w_context")}
                    value={weights.context}
                    onChange={v => setW("context", v)}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className="grid place-items-center h-9 w-9 rounded-lg font-display font-bold text-sm shrink-0"
                          style={{
                            background:
                              i < 3
                                ? "var(--primary)"
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2.5 pl-12">
                        {r.reasons.map((reason, idx) => (
                          <span
                            key={idx}
                            className="px-2 py-0.5 rounded text-[11px] bg-primary/[0.08] text-primary/90 border border-primary/15"
                          >
                            {reason}
                          </span>
                        ))}
