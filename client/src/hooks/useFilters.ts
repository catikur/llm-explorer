...
import { intelNorm, speedNorm, priceNorm, ctxNorm } from "@/components/MetricBits";

export type SortKey =
  | "intelligence" | "speed" | "price" | "value" | "context" | "name" | "vendor";
...
export const EMPTY_FILTERS: FilterState = {
  q: "", usecases: [], vendors: [], tiers: [], modality: null, caps: [],
  maxPrice: null, onlyFree: false, onlyBench: false,
};
...
export function useFilters(models: LLMModel[], bounds: MetricBounds | null) {
  ...
  const toggleIn = useCallback((key: "usecases" | "vendors" | "tiers" | "caps", val: string) => {
    setFilters((prev) => {
      const arr = prev[key];
  ...
  const filtered = useMemo(() => {
    ...
    return models.filter((m) => {
      if (q && !m.name.toLowerCase().includes(q) && !m.vendor.toLowerCase().includes(q) && !m.id.toLowerCase().includes(q))
        return false;
      if (filters.usecases.length && !filters.usecases.some((u) => m.usecases.includes(u))) return false;
      ...
      if (filters.modality) {
        ...
        if (mod === "video" && !m.input_modalities.includes("video")) return false;
  ...
  const sorted = useMemo(() => {
    ...
    const val = (m: LLMModel): number | string => {
      switch (sortKey) {
        ...
        case "intelligence": return intelNorm(m, bounds) ?? -1;
        case "speed": return speedNorm(m, bounds) ?? -1;
        case "context": return ctxNorm(m, bounds) ?? -1;
        case "value": return m.value_score ?? -1;
        case "price": {
  ...
  return { filters, setF, toggleIn, reset, filtered: sorted, rawCount: filtered.length, sortKey, sortDir, toggleSort };
}
